 ...
 *
 * 选项:
 *   --model <v30|pro>       视频模型 (默认: v30)
 *   --req-key <值>          自定义 req_key（优先级高于 --model）
...
function parseArgs(): Text2VideoOptions {
  ...
  if (args.length < 1) {
    ...
    console.error('');
    console.error('选项:');
    console.error('  --model <v30|pro>       视频模型 (默认: v30)');
    console.error('  --req-key <值>          自定义 req_key（优先级高于 --model）');
  ...
  for (let i = 1; i < args.length; i++) {
    switch (args[i]) {
      case '--model':
        const m = args[++i];
        if (m !== 'v30' && m !== 'pro') {
          throw new Error('model 必须是 v30 或 pro');
        }
        model = m;
        break;
      case '--req-key':
  ...
  return { prompt, model, reqKey, ratio, duration, fps, outputDir, download, wait, debug };
}
...
async function main(): Promise<void> {
  try {
    ...
    // 文生视频 req_key：优先使用用户传入，其次按模型映射
    const reqKeyMap = {
      v30: REQ_KEYS.T2V_V30_1080P,
      pro: REQ_KEYS.TI2V_V30_PRO
    } as const;
    const reqKey = options.reqKey || reqKeyMap[options.model];
    ...
    if (fs.existsSync(taskFolderPath)) {
      const existingVideo = getVideoInFolder(taskFolderPath);
      if (existingVideo) {
        const localResult = {
          success: true,
          prompt: options.prompt,
          ...
          duration: options.duration,
          fps: options.fps,
          taskId: loadTaskId(taskFolderPath) || undefined,
          localVideo: existingVideo,
